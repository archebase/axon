...
topics:
  ...
  EXPECT_EQ(config.topics[0].message_type, "sensor_msgs/msg/Image");
}
...
topics:
  ...
  ConfigParser parser;
  RecorderConfig config;
  // Loading should fail because validation happens automatically
...
topics:
  ...
  ConfigParser parser;
  RecorderConfig config;
  ASSERT_TRUE(parser.load_from_file(path, config));
  EXPECT_EQ(config.dataset.path, "/data/recordings");
  EXPECT_TRUE(config.validate());
}
...
TEST_F(ConfigParserTest, LoadFromNonexistentFile) {
  ConfigParser parser;
  RecorderConfig config;
  ASSERT_FALSE(parser.load_from_file("/nonexistent/path.yaml", config));
  // Should return false
...
TEST_F(ConfigParserTest, SaveToFile) {
  ...
  ASSERT_TRUE(parser.load_from_file(path, loaded));
