...
std::string CallbackMetadata::to_json() const {
  std::ostringstream oss;
  oss << "{";
  oss << "\"scene\": \"" << escape_json_string(scene) << "\", ";
  oss << "\"subscene\": \"" << escape_json_string(subscene) << "\", ";
  oss << "\"skills\": " << string_array_to_json(skills) << ", ";
  oss << "\"factory\": \"" << escape_json_string(factory) << "\"";
  oss << "}";
  return oss.str();
}
...
std::string HttpClient::get_iso8601_timestamp() {
  return get_iso8601_timestamp(std::chrono::system_clock::now());
}

std::string HttpClient::get_iso8601_timestamp(std::chrono::system_clock::time_point tp) {
...
HttpCallbackResult HttpClient::http_post(
...
HttpCallbackResult HttpClient::post_start_callback(
...
HttpCallbackResult HttpClient::post_finish_callback(
...
void HttpClient::post_start_callback_async(
  ...
  std::thread([self, task_config, payload]() {
    ...
    if (!result.success) {
      std::cerr << "Start callback failed: " << result.error_message << std::endl;
    } else {
      std::cout << "Start callback sent successfully for task: " << task_config.task_id
                << std::endl;
    }
  }).detach();
}

void HttpClient::post_finish_callback_async(
  ...
  std::thread([self, task_config, payload]() {
    ...
    if (!result.success) {
      std::cerr << "Finish callback failed: " << result.error_message << std::endl;
    } else {
      std::cout << "Finish callback sent successfully for task: " << task_config.task_id
                << std::endl;
    }
  }).detach();
}
