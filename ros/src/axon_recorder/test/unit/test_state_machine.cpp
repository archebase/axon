...
TEST_F(StateManagerTest, ThreadSafety) {
  ...
  for (int t = 0; t < kNumThreads; ++t) {
    threads.emplace_back([&]() {
      for (int i = 0; i < kIterations; ++i) {
        ...
        (void)state1; (void)state2;  // Suppress unused warnings
  ...
  EXPECT_TRUE(final_state == RecorderState::IDLE || 
              final_state == RecorderState::READY ||
              final_state == RecorderState::RECORDING ||
              final_state == RecorderState::PAUSED);
  
  // Verify state string is consistent
  std::string state_str = state_manager_.get_state_string();
...
TEST_F(StateManagerTest, SelfTransitionAttempt) {
  std::string error_msg;
...
TEST_F(StateManagerTest, TransitionWithExpectedState_WrongCurrent) {
  std::string error_msg;
...
TEST_F(StateManagerTest, TransitionWithExpectedState_InvalidTransition) {
  std::string error_msg;
...
TEST_F(StateManagerTest, GetValidTransitionsFromAllStates) {
  std::string error_msg;
...
TEST_F(StateManagerTest, CallbackInvokedInCorrectOrder) {
  std::vector<int> order;
  
  state_manager_.register_transition_callback(
    [&](RecorderState, RecorderState) { order.push_back(1); });
  
  state_manager_.register_transition_callback(
    [&](RecorderState, RecorderState) { order.push_back(2); });
  
  state_manager_.register_transition_callback(
    [&](RecorderState, RecorderState) { order.push_back(3); });
...
TEST_F(StateManagerTest, CallbackReceivesCorrectStates) {
...
TEST_F(StateManagerTest, FailedTransitionDoesNotInvokeCallback) {
...
TEST_F(StateManagerTest, ResetClearsState) {
  std::string error_msg;
...
TEST_F(StateManagerTest, IsRecordingActiveAllStates) {
  std::string error_msg;
...
TEST_F(StateManagerTest, ConcurrentReadsAndWrites) {
