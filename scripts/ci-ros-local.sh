#!/bin/bash
# =============================================================================
# Local CI Script for ROS Tests
# =============================================================================
# Mirrors .github/workflows/tests-ros.yml for local testing.
# Runs ROS unit and integration tests for all distributions.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "This script mirrors the GitHub Actions workflow for ROS tests."
            echo ""
            echo "Prerequisites:"
...
echo -e "${BLUE}=============================================${NC}"
echo -e "${BLUE}Axon ROS Tests Local CI${NC}"
echo -e "${BLUE}=============================================${NC}"
echo ""
...
test_distro() {
    ...
    local COVERAGE_DIR="${PROJECT_ROOT}/coverage_output_${DISTRO}"
    ...
    if [ "$RUN_COVERAGE" = true ]; then
        DOCKER_CMD="${DOCKER_CMD} /workspace/axon/docker/scripts/run_integration.sh --coverage --coverage-output /coverage_output"
    else
        DOCKER_CMD="${DOCKER_CMD} /workspace/axon/docker/scripts/run_integration.sh"
    fi
