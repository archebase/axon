...
bool ServiceAdapter::register_services() {
  ...
  // Register services with namespaced names (using middleware plugin interfaces)
  cached_config_server_ = node->create_service<ros2_plugin::srv::CachedRecordingConfig>(
  ...
  is_ready_server_ = node->create_service<ros2_plugin::srv::IsRecordingReady>(
    "~/is_recording_ready",
    std::bind(
  ...
  control_server_ = node->create_service<ros2_plugin::srv::RecordingControl>(
    "~/recording_control",
    std::bind(
  ...
  status_server_ = node->create_service<ros2_plugin::srv::RecordingStatus>(
    "~/recording_status",
    std::bind(
