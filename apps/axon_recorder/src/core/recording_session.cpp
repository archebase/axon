...
void RecordingSession::close() {
  ...
  // 3. Capture close time and true on-disk file size AFTER writer_->close().
  close_time_ = std::chrono::system_clock::now();
  {
    std::error_code ec;
    ...
    if (ec) {
      ...
      AXON_LOG_WARN(
        "Failed to get file size after close" << axon::logging::kv("path", output_path_)
