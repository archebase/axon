...
RecorderNode::RecorderNode()
    ...
    , worker_pool_(std::make_unique<axon::utils::WorkerThreadPool>())
    , http_callback_client_(std::make_shared<HttpClient>())
    , state_machine_(
        RecorderState::IDLE,
        std::unordered_map<RecorderState, std::vector<RecorderState>>{
...
void RecorderNode::setup_topic_recording(const TopicConfig& topic_config) {
  ...
  axon::utils::WorkerThreadPool::MessageHandler handler =
    [session_ptr, channel_id, topic_name, msg_type](
  ...
  TopicManager::MessageCallback callback =
    [this,
     topic_name](const std::string& topic, int64_t timestamp_ns, std::vector<uint8_t>&& data) {
      ...
      axon::utils::MessageItem item(timestamp_ns, std::move(data));
...
bool RecorderNode::start_recording(std::string& error_msg) {
  // Note: State validation is done by RecordingServiceImpl before calling this method
  // StateMachine is the single source of truth for recording state
  ...
  if (config_opt && config_opt->has_callbacks()) {
    ...
    payload.started_at = HttpClient::get_iso8601_timestamp(recording_start_time_);
...
bool RecorderNode::pause_recording(std::string& error_msg) {
  // Note: State validation is done by RecordingServiceImpl before calling this method
  // StateMachine is the single source of truth for recording state
...
bool RecorderNode::resume_recording(std::string& error_msg) {
  // Note: State validation is done by RecordingServiceImpl before calling this method
  // StateMachine is the single source of truth for recording state
...
bool RecorderNode::cancel_recording(std::string& error_msg) {
  AXON_LOG_INFO("cancel_recording() called");

  // Note: State is managed by StateMachine - no need to set flags here
  ...
  if (config_opt && config_opt->has_callbacks()) {
    ...
    payload.started_at = HttpClient::get_iso8601_timestamp(recording_start_time_);
    payload.finished_at = HttpClient::get_iso8601_timestamp(now);
...
double RecorderNode::get_recording_duration_sec() const {
  // Use StateMachine to check if recording is active
  if (!is_recording()) {
    return 0.0;
  }
  auto now = std::chrono::system_clock::now();
...
std::string RecorderNode::get_state_string() const {
  return state_to_string(state_machine_.get_state());
}

bool RecorderNode::is_recording_active() const {
  return is_recording();
}

std::optional<TaskConfig> RecorderNode::get_cached_config() const {
  return task_config_cache_.get();
}

bool RecorderNode::get_recording_metrics(
  ...
  duration_sec = get_recording_duration_sec();

  auto stats = get_stats();
  message_count = stats.messages_written;

  // Calculate disk usage
  if (!output_path_.empty() && std::filesystem::exists(output_path_)) {
    auto file_size = std::filesystem::file_size(output_path_);
    disk_usage_gb = static_cast<double>(file_size) / (1024.0 * 1024.0 * 1024.0);
  } else {
    disk_usage_gb = 0.0;
  }
  ...
  if (duration_sec > 0.0) {
    throughput_mb_sec = (disk_usage_gb * 1024.0) / duration_sec;
  } else {
    throughput_mb_sec = 0.0;
  }

  // Get last error from recording session
  if (recording_session_) {
    last_error = recording_session_->get_last_error();
  } else {
    last_error.clear();
  }
...
bool RecorderNode::cache_task_config(const TaskConfig& config, std::string& error_msg) {
  std::string error;
  ...
  task_config_cache_.cache(config);
  return true;
}

bool RecorderNode::finish_recording(std::string& error_msg) {
  stop_recording();
...
bool RecorderNode::clear_config(std::string& error_msg) {
  std::string error;
