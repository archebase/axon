...
TEST_F(RecorderNodeRecordingTest, HttpCallbackClientAvailable) {
  ...
  std::string timestamp = HttpClient::get_iso8601_timestamp(now);
...
protected:
  void SetUp() override {
    ...
    string_pub_ = publisher_node_->create_publisher<std_msgs::msg::String>("/test_string", 10);
    int_pub_ = publisher_node_->create_publisher<std_msgs::msg::Int32>("/test_int", 10);
  }
...
TEST_F(RecorderNodeFullRecordingTest, WriteStatsFileCreatesValidJson) {
  ...
  ASSERT_TRUE(sm.transition_to(RecorderState::READY, error))
    << "Failed to transition to READY: " << error;

  // Start recording
  ASSERT_TRUE(sm.transition_to(RecorderState::RECORDING, error))
  ...
  node->stop_recording();
  ASSERT_TRUE(sm.transition_to(RecorderState::IDLE, error))
