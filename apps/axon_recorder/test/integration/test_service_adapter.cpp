...
protected:
  void SetUp() override {
    ...
    auto config =
      create_test_config(::testing::UnitTest::GetInstance()->current_test_info()->name());
...
TEST_F(ServiceAdapterTest, IsRecordingReadyService_NotConfigured) {
  ...
  // Note: is_configured reflects TaskConfigCache state (READY/RECORDING/PAUSED), not RecorderConfig
  // injection
...
TEST_F(ServiceAdapterTest, CachedRecordingConfigServiceROS1) {
  ros::ServiceClient client = nh_->serviceClient<axon_recorder::CachedRecordingConfig>(
    "/axon_recorder/cached_recording_config"
  );
