...
TEST_F(StateTransactionGuardTest, CommitPreventsRollback) {
  ...
  // Transition to READY
  std::string error;
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::READY, error));

  {
    // Create guard with IDLE as rollback state
    StateTransactionGuard guard(*state_machine_->, RecorderState::IDLE);

    // Transition to RECORDING
    ASSERT_TRUE(state_machine_->transition_to(RecorderState::RECORDING, error));
  ...
  EXPECT_TRUE(state_machine_->is_state(RecorderState::RECORDING));
}
...
TEST_F(StateTransactionGuardTest, GetRollbackState) {
  StateTransactionGuard guard(*state_machine_->, RecorderState::READY);
...
TEST_F(StateTransactionGuardTest, UseInTypicalServicePattern) {
  ...
  auto start_recording = [this]() -> bool {
    RecorderState previous_state = state_machine_->get_state();
    StateTransactionGuard guard(*state_machine_->, previous_state);

    std::string err;
    if (!state_machine_->transition_to(RecorderState::RECORDING, err)) {
...
TEST_F(StateTransactionGuardTest, NestedGuards) {
...

  {
    ...
    StateTransactionGuard outer_guard(*state_machine_->, RecorderState::IDLE);
...

    {
      ...
      StateTransactionGuard inner_guard(*state_machine_->, RecorderState::READY);
