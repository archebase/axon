/**
 * Unit tests for RecorderState and StateMachine
 */
...
protected:
  void SetUp() override {
    ...
    state_machine_ = std::make_unique<StateMachine>(RecorderState::IDLE, transitions);
  }
...
TEST_F(StateMachineTest, InitialState) {
  EXPECT_EQ(state_machine_->get_state(), RecorderState::IDLE);
  EXPECT_EQ(state_to_string(state_machine_->get_state()), "idle");
  EXPECT_TRUE(state_machine_->is_state(RecorderState::IDLE));
  EXPECT_FALSE(state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED}));
}

TEST_F(StateMachineTest, ValidTransitionIdleToReady) {
  std::string error_msg;
  EXPECT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));
  EXPECT_TRUE(error_msg.empty());
  EXPECT_EQ(state_machine_->get_state(), RecorderState::READY);
}

TEST_F(StateMachineTest, ValidTransitionReadyToRecording) {
  std::string error_msg;
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));

  EXPECT_TRUE(state_machine_->transition_to(RecorderState::RECORDING, error_msg));
  EXPECT_TRUE(error_msg.empty());
  EXPECT_EQ(state_machine_->get_state(), RecorderState::RECORDING);
  EXPECT_TRUE(state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED}));
}

TEST_F(StateMachineTest, ValidTransitionRecordingToPaused) {
  std::string error_msg;
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::RECORDING, error_msg));

  EXPECT_TRUE(state_machine_->transition_to(RecorderState::PAUSED, error_msg));
  EXPECT_TRUE(error_msg.empty());
  EXPECT_EQ(state_machine_->get_state(), RecorderState::PAUSED);
  EXPECT_TRUE(state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED}));
}
...
TEST_F(StateMachineTest, ValidTransitionPausedToRecording) {
  ...
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::RECORDING, error_msg));
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::PAUSED, error_msg));
...
TEST_F(StateMachineTest, ValidTransitionRecordingToIdle) {
  ...
  EXPECT_TRUE(state_machine_->transition_to(RecorderState::IDLE, error_msg));
  EXPECT_TRUE(error_msg.empty());
  EXPECT_EQ(state_machine_->get_state(), RecorderState::IDLE);
  EXPECT_FALSE(state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED}));
}
...
TEST_F(StateMachineTest, InvalidTransitionIdleToRecording) {
  std::string error_msg;
  EXPECT_FALSE(state_machine_->transition_to(RecorderState::RECORDING, error_msg));
  EXPECT_FALSE(error_msg.empty());
  EXPECT_TRUE(error_msg.find("ERR_INVALID_STATE") != std::string::npos);
  EXPECT_EQ(state_machine_->get_state(), RecorderState::IDLE);  // State unchanged
}

TEST_F(StateMachineTest, InvalidTransitionIdleToPaused) {
  std::string error_msg;
  EXPECT_FALSE(state_machine_->transition_to(RecorderState::PAUSED, error_msg));
  EXPECT_FALSE(error_msg.empty());
  EXPECT_EQ(state_machine_->get_state(), RecorderState::IDLE);
}
...
TEST_F(StateMachineTest, TransitionWithExpectedState) {
  ...
  EXPECT_TRUE(state_machine_->transition(RecorderState::IDLE, RecorderState::READY, error_msg));
  EXPECT_TRUE(error_msg.empty());

  // Wrong expected state
  EXPECT_FALSE(state_machine_->transition(RecorderState::IDLE, RecorderState::RECORDING, error_msg)
  );
  EXPECT_FALSE(error_msg.empty());
...
TEST_F(StateMachineTest, IsValidTransition) {
  ...
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::IDLE, RecorderState::READY));
  EXPECT_FALSE(state_machine_->is_valid_transition(RecorderState::IDLE, RecorderState::RECORDING));
  EXPECT_FALSE(state_machine_->is_valid_transition(RecorderState::IDLE, RecorderState::PAUSED));

  // Valid transitions from READY
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::READY, RecorderState::RECORDING));
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::READY, RecorderState::IDLE));
  EXPECT_FALSE(state_machine_->is_valid_transition(RecorderState::READY, RecorderState::PAUSED));

  // Valid transitions from RECORDING
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::RECORDING, RecorderState::PAUSED));
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::RECORDING, RecorderState::IDLE));
  EXPECT_FALSE(state_machine_->is_valid_transition(RecorderState::RECORDING, RecorderState::READY));

  // Valid transitions from PAUSED
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::PAUSED, RecorderState::RECORDING));
  EXPECT_TRUE(state_machine_->is_valid_transition(RecorderState::PAUSED, RecorderState::IDLE));
  EXPECT_FALSE(state_machine_->is_valid_transition(RecorderState::PAUSED, RecorderState::READY));
}

TEST_F(StateMachineTest, GetValidTransitions) {
  // From IDLE
  auto transitions = state_machine_->get_valid_transitions();
  ...
  std::string error_msg;
  ASSERT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));
  transitions = state_machine_->get_valid_transitions();
...
TEST_F(StateMachineTest, TransitionCallback) {
  ...
  state_machine_->register_transition_callback([&](RecorderState from, RecorderState to) {
    transitions.emplace_back(from, to);
  });

  std::string error_msg;
  state_machine_->transition_to(RecorderState::READY, error_msg);
  state_machine_->transition_to(RecorderState::RECORDING, error_msg);
  state_machine_->transition_to(RecorderState::PAUSED, error_msg);
  state_machine_->transition_to(RecorderState::IDLE, error_msg);
...
TEST_F(StateMachineTest, MultipleCallbacks) {
  ...
  state_machine_->register_transition_callback([&](RecorderState, RecorderState) {
    callback1_count++;
  });
...
TEST_F(StateMachineTest, ThreadSafety) {
  ...
  for (int t = 0; t < kNumThreads; ++t) {
    threads.emplace_back([&]() {
      for (int i = 0; i < kIterations; ++i) {
        ...
        if (state_machine_->transition_to(RecorderState::READY, error_msg)) {
          success_count++;
        } else {
          failure_count++;
        }

        if (state_machine_->transition_to(RecorderState::RECORDING, error_msg)) {
          success_count++;
        } else {
          failure_count++;
        }

        if (state_machine_->transition_to(RecorderState::IDLE, error_msg)) {
          success_count++;
        } else {
          failure_count++;
        }
        ...
        auto state1 = state_machine_->get_state();
        auto state2 = state_machine_->get_state();
        (void)state1;
        (void)state2;  // Suppress unused warnings
        state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED});
      }
    });
  }
  ...
  auto final_state = state_machine_->get_state();
  EXPECT_TRUE(
  ...
  std::string state_str = state_to_string(state_machine_->get_state());
...
TEST_F(StateMachineTest, TypicalRecordingWorkflow) {
  ...
  EXPECT_TRUE(state_machine_->transition_to(RecorderState::READY, error_msg));
  EXPECT_TRUE(state_machine_->is_state(RecorderState::READY));
  ...
  EXPECT_TRUE(state_machine_->transition_to(RecorderState::PAUSED, error_msg));
  EXPECT_TRUE(state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED}));
  EXPECT_TRUE(state_machine_->is_state(RecorderState::PAUSED));
...
TEST_F(StateMachineTest, SelfTransitionAttempt) {
  ...
  EXPECT_FALSE(state_machine_->transition_to(RecorderState::IDLE, error_msg));
  EXPECT_FALSE(error_msg.empty());
  EXPECT_TRUE(state_machine_->is_state(RecorderState::IDLE));
  ...
  EXPECT_FALSE(state_machine_->transition_to(RecorderState::READY, error_msg));
  EXPECT_TRUE(state_machine_->is_state(RecorderState::READY));
}
...
TEST_F(StateMachineTest, TransitionWithExpectedState_WrongCurrent) {
  ...
  EXPECT_FALSE(state_machine_->transition(RecorderState::READY, RecorderState::RECORDING, error_msg)
  );
  EXPECT_TRUE(error_msg.find("Expected state") != std::string::npos);
...
TEST_F(StateMachineTest, TransitionWithExpectedState_InvalidTransition) {
  ...
  EXPECT_TRUE(error_msg.find("Cannot transition") != std::string::npos);
}
...
TEST_F(StateMachineTest, GetValidTransitionsFromAllStates) {
  ...
  // IDLE state
  auto idle_transitions = state_machine_->get_valid_transitions();
  EXPECT_EQ(idle_transitions.size(), 1);
  EXPECT_EQ(idle_transitions[0], RecorderState::READY);

  // READY state
  state_machine_->transition_to(RecorderState::READY, error_msg);
  auto ready_transitions = state_machine_->get_valid_transitions();
  EXPECT_EQ(ready_transitions.size(), 2);

  // RECORDING state
  state_machine_->transition_to(RecorderState::RECORDING, error_msg);
  auto recording_transitions = state_machine_->get_valid_transitions();
  EXPECT_EQ(recording_transitions.size(), 2);

  // PAUSED state
  state_machine_->transition_to(RecorderState::PAUSED, error_msg);
  auto paused_transitions = state_machine_->get_valid_transitions();
  ...
  for (int i = 0; i < num_readers; ++i) {
    threads.emplace_back([&]() {
      while (!stop.load()) {
        ...
        state_to_string(state_machine_->get_state());
        state_machine_->is_state_in({RecorderState::RECORDING, RecorderState::PAUSED});
        state_machine_->get_valid_transitions();
