...
struct PublishStats {
  ...
  void write_stats_file() {
    ...
    stats_file << "{\n"
               ...
               << "  \"throughput_mbps\": " << std::fixed << std::setprecision(2)
...
sensor_msgs::msg::Image generate_rgb_image(
  rclcpp::Node& node, int width, int height, int camera_id, std::mt19937& rng
) {
...
sensor_msgs::msg::Image generate_depth_image(
  rclcpp::Node& node, int width, int height, int camera_id, std::mt19937& rng
) {
  sensor_msgs::msg::Image msg;
  msg.header.stamp = node.now();
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < config.num_cameras; ++i) {
    rgb_pubs.push_back(nh.advertise<sensor_msgs::Image>("/camera" + std::to_string(i) + "/rgb", 10)
    );
  ...
  std::thread camera_thread([&]() {
    ...
    while (running && ros::ok()) {
      for (int i = 0; i < config.num_cameras; ++i) {
        auto rgb_msg = axon::test::generate_rgb_image(config.width, config.height, i, rng);
        rgb_pubs[i].publish(rgb_msg);
        stats.rgb_published++;
        stats.total_bytes += rgb_msg.data.size();

        auto depth_msg = axon::test::generate_depth_image(config.width, config.height, i, rng);
...
public:
  SyntheticPublisher(const axon::test::PublisherConfig& config)
      ...
      , rng_(42)
      , running_(true) {
    // Note: stats_.start_time is set AFTER subscriber discovery (see below)
    ...
    // Create per-camera publishers first (timers created after discovery)
    ...
    // Wait for subscriber discovery before starting timers
    // This prevents message loss due to DDS discovery race condition
    RCLCPP_INFO(get_logger(), "Waiting for subscribers to be discovered...");
    wait_for_subscribers();
    RCLCPP_INFO(get_logger(), "All subscribers discovered, starting publishers");

    // Warmup phase: Send a few messages to prime the DDS reliable transport
    // This ensures the reliable connection is fully established before measuring
    RCLCPP_INFO(get_logger(), "Warmup: priming DDS reliable transport...");
    warmup_publish();

    // Start timing AFTER warmup completes
    // This ensures test duration only measures steady-state publishing
    stats_.start_time = std::chrono::steady_clock::now();

    // Now create the timers (after subscribers are ready)
    ...
    imu_timer_ = create_wall_timer(
      imu_period, std::bind(&SyntheticPublisher::imu_callback, this), imu_callback_group_
    );

    // Status timer (runs on default callback group)
    status_timer_ = create_wall_timer(
      std::chrono::seconds(1), std::bind(&SyntheticPublisher::status_callback, this)
    );

    // Duration timer
    duration_timer_ = create_wall_timer(std::chrono::seconds(config_.duration_sec), [this]() {
...
private:
  ...
  /**
   * Wait for subscribers to be discovered before publishing
   * This prevents message loss due to DDS discovery race conditions
   */
  void wait_for_subscribers() {
    ...
    while (rclcpp::ok()) {
      ...
      // Check IMU subscriber
      if (imu_pub_->get_subscription_count() == 0) {
        all_found = false;
      }

      // Check all camera subscribers
      for (size_t i = 0; i < rgb_pubs_.size(); ++i) {
        if (rgb_pubs_[i]->get_subscription_count() == 0 || depth_pubs_[i]->get_subscription_count() == 0) {
          all_found = false;
          break;
        }
      }
      ...
      if (all_found) {
        RCLCPP_INFO(
          get_logger(),
          "All %zu publishers have subscribers",
          1 + rgb_pubs_.size() * 2
        );  // IMU + (RGB + Depth) * num_cameras

        // CRITICAL: Wait for DDS reliable connection to fully establish
        // Discovery != connection ready. DDS needs time to complete the
        // reliable transport handshake after discovery.
        RCLCPP_INFO(get_logger(), "Waiting for DDS connections to stabilize...");
      ...
      if (std::chrono::steady_clock::now() - start > timeout) {
        RCLCPP_WARN(get_logger(), "Timeout waiting for subscribers, proceeding anyway");
        return;
      }
      ...
      auto elapsed =
        std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start)
          .count();
      if (elapsed % 2 == 0) {
        size_t imu_subs = imu_pub_->get_subscription_count();
        size_t rgb_subs = 0, depth_subs = 0;
        for (size_t i = 0; i < rgb_pubs_.size(); ++i) {
          rgb_subs += rgb_pubs_[i]->get_subscription_count();
          depth_subs += depth_pubs_[i]->get_subscription_count();
        }
        RCLCPP_INFO(
          get_logger(),
          "Waiting for subscribers... IMU: %zu, RGB: %zu/%zu, Depth: %zu/%zu",
          imu_subs,
          rgb_subs,
          rgb_pubs_.size(),
          depth_subs,
  ...
  /**
   * Warmup phase to prime DDS reliable transport
   * Sends a small number of messages on all topics to ensure:
   * 1. DDS reliable handshake is complete
   * 2. Memory pools are allocated
   * 3. Any initial latency spikes are absorbed
   */
  void warmup_publish() {
    const int WARMUP_ITERATIONS = 10;
    const auto WARMUP_INTERVAL = std::chrono::milliseconds(50);
    ...
    for (int iter = 0; iter < WARMUP_ITERATIONS; ++iter) {
      ...
      // Publish all cameras
      for (size_t i = 0; i < rgb_pubs_.size(); ++i) {
        rgb_templates_[i].header.stamp = now();
        rgb_pubs_[i]->publish(rgb_templates_[i]);

        depth_templates_[i].header.stamp = now();
        depth_pubs_[i]->publish(depth_templates_[i]);
      }

      std::this_thread::sleep_for(WARMUP_INTERVAL);
    }

    // Final delay to ensure all warmup messages are ACKed
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    RCLCPP_INFO(get_logger(), "Warmup complete (%d messages per topic)", WARMUP_ITERATIONS);
  }
