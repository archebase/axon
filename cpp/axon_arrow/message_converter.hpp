...
private:
  std::shared_ptr<arrow::Array> convert_field(
    const void* message, const struct FieldDescriptor& field_desc
  );
  std::shared_ptr<arrow::Array> convert_builtin_field(
    const void* message, const struct FieldDescriptor& field_desc
  );
...
private:
  ...
  bool convert_impl(const MessageType& msg, std::vector<std::shared_ptr<arrow::Array>>& arrays) {
