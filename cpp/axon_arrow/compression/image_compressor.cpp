...
bool NoCompressor::compress(const ImageData& input,
...
bool NoCompressor::decompress(const uint8_t* input,
                             size_t input_size,
                             uint8_t* output,
                             size_t output_size) {
...
bool Lz4Compressor::compress(const ImageData& input,
                            std::vector<uint8_t>& output,
                            CompressionStats* stats) {
    if (!codec_) {
        return false;
...
bool Lz4Compressor::decompress(const uint8_t* input,
                              size_t input_size,
                              uint8_t* output,
                              size_t output_size) {
    if (!codec_) {
...
bool ZstdCompressor::compress(const ImageData& input,
                             std::vector<uint8_t>& output,
                             CompressionStats* stats) {
    if (!codec_) {
        return false;
...
bool ZstdCompressor::decompress(const uint8_t* input,
                               size_t input_size,
                               uint8_t* output,
                               size_t output_size) {
    if (!codec_) {
...
bool JpegCompressor::compress(const ImageData& input,
                             std::vector<uint8_t>& output,
                             CompressionStats* stats) {
    ...
    if (result != 0) {
        std::cerr << "TurboJPEG compression failed: " << tjGetErrorStr2(ctx_->compress_handle) << std::endl;
...
bool JpegCompressor::decompress(const uint8_t* input,
                               size_t input_size,
                               uint8_t* output,
                               size_t output_size) {
#if HAS_TURBOJPEG
...
bool AdaptiveCompressor::compress_rgb(const ImageData& input,
                                      std::vector<uint8_t>& output,
                                      CompressionStats* stats) {
...
bool AdaptiveCompressor::compress_depth(const ImageData& input,
                                        std::vector<uint8_t>& output,
                                        CompressionStats* stats) {
...
arrow::Result<std::shared_ptr<arrow::BinaryArray>> compress_binary_array(
    const std::shared_ptr<arrow::BinaryArray>& input,
    ...
    for (int64_t i = 0; i < input->length(); ++i) {
        ...
        ImageData img(reinterpret_cast<const uint8_t*>(value.data()), 
                     value.size(), width, height, channels);
...
CompressionBenchmarkResult benchmark_compressor(
    ...
    compressor.decompress(compressed.data(), compressed.size(), 
    ...
    result.compress_throughput_mbps = (test_data.size / 1024.0 / 1024.0) / 
                                      (result.compress_time_ms / 1000.0);
    ...
    result.decompress_time_ms = std::chrono::duration<double, std::milli>(end - start).count() / iterations;
    result.decompress_throughput_mbps = (test_data.size / 1024.0 / 1024.0) / 
                                        (result.decompress_time_ms / 1000.0);
