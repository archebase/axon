...
struct ImageData {
    ...
    ImageData(const uint8_t* d, size_t s, int w, int h, int c)
        : data(d), size(s), width(w), height(h), channels(c) {}
...
public:
     ...
     */
    virtual bool compress(const ImageData& input,
                         std::vector<uint8_t>& output,
                         CompressionStats* stats = nullptr) = 0;
     ...
     * @return true on success
     */
    virtual bool decompress(const uint8_t* input,
                           size_t input_size,
                           uint8_t* output,
...
public:
    bool compress(const ImageData& input,
                 std::vector<uint8_t>& output,
                 CompressionStats* stats = nullptr) override;
    
    bool decompress(const uint8_t* input,
                   size_t input_size,
                   uint8_t* output,
                   size_t output_size) override;
    
    CompressionCodec codec() const override { return CompressionCodec::NONE; }
    std::string codec_name() const override { return "none"; }
};
...
public:
    ...
    CompressionCodec codec() const override { return CompressionCodec::LZ4; }
...
public:
    ...
    CompressionCodec codec() const override { return CompressionCodec::ZSTD; }
    std::string codec_name() const override { return "zstd"; }
...
public:
    ...
    CompressionCodec codec() const override { return CompressionCodec::JPEG; }
    std::string codec_name() const override { return "jpeg"; }
    
    void set_quality(int quality) { quality_ = std::clamp(quality, 1, 100); }
...
public:
     ...
     */
    bool compress_rgb(const ImageData& input,
                     std::vector<uint8_t>& output,
     ...
     */
    bool compress_depth(const ImageData& input,
                       std::vector<uint8_t>& output,
                       CompressionStats* stats = nullptr);
     ...
     */
    const CompressionStats& rgb_stats() const { return rgb_stats_; }
    
    /**
     * Get current depth compressor stats
     */
    const CompressionStats& depth_stats() const { return depth_stats_; }
