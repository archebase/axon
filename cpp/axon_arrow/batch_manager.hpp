...
public:
  struct BatchItem {
    ...
    BatchItem(std::shared_ptr<arrow::RecordBatch> b, const std::string& path, int64_t handle)
        : batch(std::move(b))
        , dataset_path(path)
  ...
  ~BatchQueue() {
    stop();
    // Clear remaining batches to release Arrow resources
    clear();
  }

  /**
   * Clear all remaining items in the queue
   * Must be called while Arrow memory pool is still valid
   */
  void clear() {
...
public:
  using WriteCallback =
    std::function<bool(const std::shared_ptr<arrow::RecordBatch>&, const std::string&, int64_t)>;
  ...
  // Disable copy and move (BatchQueue contains std::mutex which is not movable)
