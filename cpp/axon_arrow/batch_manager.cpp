...
BatchManager::BatchManager(
  size_t batch_size, int flush_interval_ms, WriteCallback write_callback,
...
BatchManager::~BatchManager() {
  // Ensure all resources are released - stop() is idempotent
  if (running_.load(std::memory_order_acquire)) {
    stop();
  }

  // Clear any remaining Arrow resources
  // This is a safety measure in case stop() wasn't called explicitly
  ...
  for (int i = 0; i < schema_->num_fields(); ++i) {
    ...
    if (!builder.ok()) {
      throw std::runtime_error(
        "Failed to create builder for field " + field->name() + ": " + builder.status().ToString()
      );
    }
...
void BatchManager::add_row(const std::vector<std::shared_ptr<arrow::Array>>& arrays) {
  ...
  if (arrays.size() != static_cast<size_t>(schema_->num_fields())) {
    throw std::runtime_error(
      "Array count mismatch. Expected " + std::to_string(schema_->num_fields()) + ", got " +
  ...
  for (size_t i = 0; i < arrays.size(); ++i) {
    ...
    if (!status.ok()) {
      std::cerr << "Warning: Failed to append to builder " << i << ": " << status.ToString()
                << std::endl;
...
void BatchManager::add_row_with_metadata(
  int64_t timestamp_ns, const std::string& topic_name,
  ...
  if (data_arrays.size() != expected_data_fields) {
    throw std::runtime_error(
      "Data array count mismatch. Expected " + std::to_string(expected_data_fields) + ", got " +
      std::to_string(data_arrays.size())
    );
  }
  ...
  for (size_t i = 0; i < data_arrays.size(); ++i) {
    ...
    arrow::Status status =
      builders_[builder_idx]->AppendArraySlice(span, 0, data_arrays[i]->length());
    if (!status.ok()) {
...
void BatchManager::add_row_with_metadata(
  int64_t timestamp_ns, const std::string& topic_name, std::vector<uint8_t>&& raw_data,
  const std::vector<std::shared_ptr<arrow::Array>>& data_arrays
) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void BatchManager::add_row_with_raw_data(
  int64_t timestamp_ns, const std::string& topic_name, std::vector<uint8_t>&& raw_data
) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void BatchManager::flush_internal() {
  ...
  for (size_t i = 0; i < builders_.size(); ++i) {
    ...
    if (!status.ok()) {
      std::cerr << "Failed to finish builder " << i << ": " << status.ToString() << std::endl;
      ...
      if (null_array.ok()) {
        ...
        throw std::runtime_error("Failed to create null array: " + null_array.status().ToString());
      }
    } else {
...
void BatchManager::stop() {
  ...
  // Clear any remaining batches in the queue to release Arrow resources
  queue_.clear();

  // Explicitly release all Arrow resources to prevent static destruction issues
  // This must happen while Arrow's memory pool is still valid
  {
    std::lock_guard<std::mutex> lock(mutex_);
    builders_.clear();
    pending_raw_data_.clear();
