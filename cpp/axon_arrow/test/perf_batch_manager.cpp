...
std::vector<std::shared_ptr<arrow::Array>> create_image_row(
  int64_t timestamp, int camera_id, size_t rgb_size, size_t depth_size
) {
  std::vector<std::shared_ptr<arrow::Array>> arrays;
  ...
  for (auto _ : state) {
    ...
    for (int frame = 0; frame < 30; ++frame) {
      for (int cam = 0; cam < 3; ++cam) {
        auto row = create_image_row(frame * 33333333 + cam * 1000000, cam, rgb_size, depth_size);
        manager.add_row(row);
      }
    }
...
double get_cpu_usage() {
  ...
  if (host_statistics(mach_host_self(), HOST_CPU_LOAD_INFO, (host_info_t)&cpu_load, &count) !=
      KERN_SUCCESS) {
    return 0.0;
  }
...
BENCHMARK(BM_CPU_Usage_Recording)->Unit(benchmark::kMillisecond)->Iterations(5);
