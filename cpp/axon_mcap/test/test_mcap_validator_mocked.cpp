...
TEST_F(McapValidatorMockedTest, HeaderReadFailure) {
  ...
  EXPECT_CALL(*mock_stream, good()).WillOnce(Return(false));  // Read failed (short-circuits, gcount() not called)
...
TEST_F(McapValidatorMockedTest, FooterSeekFailure) {
  ...
  auto mock_stream = std::make_unique<MockFileStream>();
  EXPECT_CALL(*mock_stream, seekg(_, std::ios::end))
      .WillOnce(ReturnRef(*mock_stream));
...
TEST_F(McapValidatorMockedTest, FooterFileTooSmall) {
  ...
  EXPECT_CALL(*mock_filesystem_, exists(test_path_)).WillOnce(Return(true));
  EXPECT_CALL(*mock_filesystem_, file_size(test_path_)).WillOnce(Return(10));  // Less than MIN_MCAP_SIZE (16)
...
TEST_F(McapValidatorMockedTest, StructureReaderOpenFailure) {
  ...
  auto result = validateMcapStructureImpl(
      test_path_, *mock_filesystem_, *mock_stream_factory_, *mock_reader_
  );
  EXPECT_FALSE(result.valid);
...
TEST_F(McapValidatorMockedTest, StructureReaderSummaryReadFailure) {
  ...
  mcap::Status summary_error = mcap::Status(mcap::StatusCode::InvalidFile, "Summary section invalid");
...
TEST_F(McapValidatorMockedTest, FooterGoodButGcountGreaterThanMagicSize) {
  ...
  // good() returns true for both seek and read, but gcount() returns value greater than MCAP_MAGIC_SIZE
  EXPECT_CALL(*mock_stream, good())
      .WillOnce(Return(true))   // Seek succeeded
